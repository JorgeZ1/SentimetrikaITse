...
def create_dashboard_view(page: ft.Page) -> ft.View:
    ...
    selected_post_title = ft.Text(
        "Selecciona un Toot para ver sus comentarios",
    ...
    if not publications:
        publications_list_view.controls.append(
            ft.Text("No se encontraron datos de Mastodon en PostgreSQL.", style=ft.TextStyle(color=Colors.GREY_500))
        )
    ...
    return ft.View(
        ...
        [
            ft.AppBar(
                title=ft.Text("🐘 Mastodon", style=ft.TextStyle(color=Colors.WHITE)), 
